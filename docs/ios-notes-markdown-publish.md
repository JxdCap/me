...
- 当前最多 9 个媒体文件。
- 当前最多 1 个视频。
- 默认 memo 模式下，Markdown 正文里的图片引用会被移除，附件统一写入 PocketBase 的 `media` 字段。
- `@note` 富格式模式下，Markdown 正文里的图片引用会保留，并改写成 PocketBase 可直接访问的文件 URL。
...
- 有 `@note`：创建 note，正文保留 Markdown，图片引用改写成 PocketBase 文件 URL。
...
PocketBase `memos` 集合需要有 `kind` 字段：
...
- `content.kind`: `memo` 或 `note`。
- `content.rewritten_images`: `@note` 模式下被改写成 PocketBase URL 的图片数量。
...
`pocketbase_markdown_rewrite_failed`：

`@note` 模式下附件已上传，但 Markdown 图片链接回写失败。用返回里的 `request_id` 查服务日志。
